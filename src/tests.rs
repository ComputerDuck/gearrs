...
mod tests {
    ...
    async fn submit_create_job_req_with_timeout() {
        ...
        let server_handle = tokio::task::spawn(async move {
            ...
            println!("ST: sleeping to trigger timeout");
            tokio::time::sleep(std::time::Duration::from_secs(1)).await;
        ...
        match job.submit_with_timeout(TEST_FUNCTION_NAME, &mut client, std::time::Duration::from_millis(200)).await {
            Err(crate::GearmanError::Timeout) => (),
            v => match v {
                Ok(_) => panic!("Expected timeout error and got a Valid JobHandle"),
                Err(err) => panic!("Expected timeout error and got error {err}"),
            },
        }

        println!("RL: Successfully timed out");
